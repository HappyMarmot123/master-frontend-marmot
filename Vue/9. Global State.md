...
### 전역 플러그인 패턴 (Event Bus)

Vuex나 Pinia와 같은 상태 관리 라이브러리를 사용하기에는 간단하지만, 여러 컴포넌트에서 공유해야 할 상태나 기능이 있을 때 사용할 수 있는 패턴입니다. 주로 Vue 2에서 `Vue.prototype`을 확장하여 사용했으며, 전역 이벤트 버스(Event Bus)나 간단한 전역 객체를 만드는 데 활용되었습니다.

#### 작동 원리 및 특징

애플리케이션의 진입점(`main.js`)에서 Vue 인스턴스가 생성되기 전에 `Vue.prototype`에 속성을 추가하면, 해당 속성은 모든 Vue 컴포넌트에서 `this`를 통해 접근할 수 있습니다. 반응성이 필요한 데이터나 메서드를 제공하기 위해, 새로운 Vue 인스턴스를 생성하여 값으로 할당합니다. 이 새로운 인스턴스는 자체적인 `data`, `methods`를 가질 수 있어 Vuex와 유사하지만 훨씬 간단한 반응형 전역 스토어 역할을 할 수 있습니다.

`main.js`에서 플러그인을 임포트하여 애플리케이션 시작 전에 로드합니다. Vuex에 비해 구조가 매우 간단하고, 특정 기능에 특화된 상태 관리에 적합합니다. `provide/inject`와 마찬가지로 데이터의 출처를 추적하기 어려워, 규모가 큰 프로젝트에서는 상태 관리가 복잡해질 수 있습니다.
...
이제 어떤 컴포넌트에서든 `this.$editor.content`로 데이터에 접근하거나 `this.$editor.updateContent('새로운 내용')`으로 메서드를 호출할 수 있습니다.
