...
- **선언적 환경 레코드**: 일반적인 변수와 함수 선언
- **객체 환경 레코드**: with 문이나 전역 스코프에서 사용
...
// 위 코드는 실행 컨텍스트 생성 단계에서 다음과 같이 처리됨
// var x = undefined;
// function hoistedFunc() { ... }
```
...
const obj = {
  ...
  method: function () {
    ...
    setTimeout(() => {
      console.log(this.name); // '객체' (렉시컬 this)
    }, 100);
  },
};
...
- 비동기 콜백은 별도의 실행 컨텍스트에서 실행
- 원래 함수의 실행 컨텍스트와는 독립적
- 이벤트 루프를 통해 관리
...
4. **this 바인딩**: 함수 호출 방식에 따른 this 값 결정
5. **메모리 관리**: 실행 컨텍스트 종료 시 메모리 정리

## 🎯 학습 포인트

1. **실행 컨텍스트는 코드 실행 환경의 추상화**
2. **생성 단계와 실행 단계로 구분되는 동작 과정**
3. **스택 구조를 통한 함수 호출 관리**
4. **렉시컬 환경과 스코프 체인의 연관성**
5. **클로저, 호이스팅, this 바인딩의 근본 원리**
