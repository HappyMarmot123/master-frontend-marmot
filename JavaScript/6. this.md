...
화살표 함수는 위와 같은 문제를 해결하기 위해 도입되었습니다. 화살표 함수는 **자신을 둘러싼 외부(렉시컬) 스코프의 this를 그대로 가져옵니다.** 즉, 함수가 어디서 호출되든 관계없이 this가 고정됩니다.
...
// 우선순위 테스트
test.call(obj1); // '객체1' (명시적 바인딩)
obj1.test = test;
obj1.test(); // '객체1' (암시적 바인딩)

const boundTest = test.bind(obj2);
boundTest(); // '객체2' (bind는 가장 강력)
```
...
const user = {
  ...
  showHobbies: function () {
    this.hobbies.forEach(function (hobby) {
      ...
      // this는 undefined (strict 모드) 또는 window
    });
  },
};

user.showHobbies(); // 에러 발생
```
...
showHobbies: function() {
    const self = this;
    this.hobbies.forEach(function(hobby) {
        console.log(`${self.name}의 취미: ${hobby}`);
    });
}
```
...
5. **bind, call, apply**로 명시적 바인딩 가능

## 🎯 모범 사례

1. **화살표 함수**를 콜백에서 사용하여 this 바인딩 유지
2. **bind**를 사용하여 함수의 this를 영구적으로 고정
3. **메서드 체이닝**을 위해 메서드에서 this 반환
4. **클래스**에서 생성자에서 이벤트 바인딩
5. **strict 모드** 사용으로 예측 가능한 this 동작
